...
| endpoint | 统一接口服务, 移动端应用中配置 `Hard Coded`           |          |
| authEP   | 用户授权服务                                            |          |
| formEP   | 表单工作流服务                                          |          |
| dataEP   | 业务数据 `api` 服务, 一般情况下与表单 `formEP` 相同 | Optional |
| workflow | 工作流服务，一般情况下与表单 `formEP` 相同            | Optional |
...
```
{
   ...
   {
      "useCache": false,
      "endpoint": "https://api.cloud.smallsaas.cn",
      "authEP": "https://api.uaas.smallsaas.cn",
      "formEP": "https://api.mock.smallsaas.cn/form",
      "dataEP": "https://api.mock.smallsaas.cn/data",
      "workflow": "https://api.wf.smallsaas.cn",
   }
}
```
