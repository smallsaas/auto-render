...
| 属性       | 说明                                                                                        | 默认值   |
| ---------- | ------------------------------------------------------------------------------------------- | -------- |
| useCache   | 页面设计数据是否缓存                                                                        | false    |
| workflowEP | 工作流服务终端，用于发起工作流，返回表单设计数据与处理表单提交数据                          |          |
| formEP     | 表单服务终端，一般情况下已在移动端应用中配置 `Hard Coded`                                 | Optional |
| dataEP     | 表单业务数据 `api` 服务终端, 一般情况下与工作流 `workflowEP` 相同，可指定不同的数据服务 | Optional |
...
```
{
   ...
   {
      "useCache": false,
      "workflowEP": "http://api.wf.smallsaas.cn",
      "formEP": "http://api.mock.smallsaas.cn/form",
      "dataEP": http://api.mock.smallsaas.cn/data"
   }
}
```
