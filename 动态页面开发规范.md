...
| authEP     | 用户授权服务终端                                                                      |          |
| formEP     | 表单服务终端，一般情况下已在移动端应用中配置 `Hard Coded`                           |          |
| dataEP     | 表单业务数据 `api` 服务终端, 一般情况下与表单 `formEP` 相同，可指定不同的数据服务 | Optional |
| workflowEP | 工作流服务终端，用于发起工作流，返回表单设计数据与处理表单提交数据                    | Optional |
...
```
{
   "globalConfig":
   {
      ...
      "workflowEP": "https://api.wf.smallsaas.cn",
   }
}
```
