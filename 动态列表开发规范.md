...
| confirmBinding          | 通过把选项列表结果通过绑定提交至 `confirmNavigation` API | {} | 
...
| multiNavigationModule.mode | 多路由模式 `[select, multiselect]`,没有配置`mode`默认为多路由, 单选或多选 |   |
...
| 属性          | 说明                   | 默认值 |
| ------------- | ---------------------- | ------ |
| multiFilter[].api  | 直接通过`api`获取选项数据 |   |
