...
## 2. JSON 配置
- 默认数据返回标准结构为 `{"data": [], "code": 200}`
- 分页返回数据标准为 `{"data": {"records": [], "total": 12}, "code": 200}`
- 由`api`返回符合组件标准的数据, 由后台提供`数据转换`网关解决方案

以下配置移除
``` 
| response                | 返回数据过滤处理                       | {}                   |
| response.list           | 列表的数据字段名称,默认为`data`        | {}                   |
...
##### 2.1.6 `multiFilterModule` 多过滤条件查询自定义组件
Deprecated
...
#### 2.3 request 配置
- `pn/pageNum` 接收的标准为字段为 `pageNum`, 除此之外字段名称不支持
- `ps/pageSize` 接收的标准为字段为 `pageSize`, 除此之外字段名称不支持
- 由后台提供解决方案
